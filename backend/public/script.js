...
async function sendAdminMessage(text) {
  ...
  if (!res.ok || data?.ok === false) {
    ...
    throw new Error("Не удалось отправить уведомление админу");
  }
  return data;
}
...
app.post("/api/withdraw/ton", auth, async (req, res) => {
  ...
  // ВАЖНО: это заявка, баланс пока НЕ списываем (как и договаривались)
  res.json({ ok: true });
});
...
app.post("/api/withdraw/gift", auth, async (req, res) => {
  ...
  // это заявка — предмет НЕ удаляем (можно поменять на удаление после подтверждения)
  res.json({ ok: true });
});
