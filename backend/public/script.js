...
const translations = {
	en: {
		...
		wheelSubtitle: 'Each spin costs 1 TON',
		spin: 'Spin',
		price: 'Price: 1 TON',
		...
		peach: 'Peach',
		crashTitle: '🚀 Crash',
		crashSubtitle: 'Withdraw money before the rocket explodes!',
		bet: 'Bet (TON)',
		play: 'Play',
		cashOut: 'Cash Out',
		yourBet: 'Your Bet:',
		potentialWin: 'Potential Win:',
		bonusesTitle: 'Bonuses and Promo Codes',
		bonusesSubtitle: 'Enter a promo code to get a bonus or discount.',
		enterPromo: 'Enter promo code',
		activate: 'Activate',
		promoSection: 'Promo Codes Section',
		promoText:
			'In this section you can enter a promo code, promo codes will be published in the Telegram channel',
		profileTitle: 'Gift Inventory',
		profileEmpty: 'You have no gifts yet',
		referralTitle: 'Referral Program',
		referralText:
			'We share part of the income with active players. Below you can create your own promo code for friends.',
		createPromo: 'Create Promo Code (soon)',
		wheelNav: 'Wheel',
		crashNav: 'Crash',
		bonusesNav: 'Bonuses',
		profileNav: 'Profile',
	...
	ru: {
		...
		wheelSubtitle: 'Каждый спин стоит 1 TON',
		spin: 'Крутить',
		price: 'Цена: 1 TON',
		...
		peach: 'Слива',
		crashTitle: '🚀 Краш',
		crashSubtitle: 'Выведите деньги до того, как ракета взорвётся!',
		bet: 'Ставка (TON)',
		play: 'Играть',
		cashOut: 'Вывести',
		yourBet: 'Ваша ставка:',
		potentialWin: 'Потенциальный выигрыш:',
		bonusesTitle: 'Бонусы и промокоды',
		bonusesSubtitle: 'Введите промокод, чтобы получить бонус или скидку.',
		enterPromo: 'Введите промокод',
		activate: 'Активировать',
		promoSection: 'Раздел промокоды',
		promoText:
			'В данном разделе можно будет ввести промокод, промокоды будут публиковаться в тг канале',
		profileTitle: 'Инвентарь подарков',
		profileEmpty: 'У вас пока нет подарков',
		...
		createPromo: 'Создать промокод (скоро)',
		wheelNav: 'Колесо',
		crashNav: 'Краш',
		bonusesNav: 'Бонусы',
		profileNav: 'Профиль',
...
document
	.getElementById('close-settings-bottom')
	?.addEventListener('click', closeSettings)
...
function applyLanguage() {
	...
	// Update currency
	const currency = document.querySelector('.balance-currency')
	if (currency) currency.textContent = t('currency')

	// Update wheel screen
	const wheelTitle = document.querySelector('#screen-wheel .screen-title')
	if (wheelTitle) wheelTitle.textContent = t('wheelTitle')

	const wheelSubtitle = document.querySelector('#screen-wheel .screen-subtitle')
	if (wheelSubtitle) wheelSubtitle.textContent = t('wheelSubtitle')

	const spinButton = document.getElementById('spin-button')
	if (spinButton) {
		spinButton.textContent = t('spin')
		const spinSubtitle = spinButton.querySelector('.spin-subtitle')
		if (spinSubtitle) spinSubtitle.textContent = t('price')
	}

	const yourBalance = document.querySelector('#screen-wheel .info-label')
	if (yourBalance) yourBalance.textContent = t('yourBalance')

	const lastPrizeLabel = document.querySelector(
		'#screen-wheel .info-card:nth-child(2) .info-label',
	)
	if (lastPrizeLabel) lastPrizeLabel.textContent = t('lastPrize')
	...
	if (prizeNames.length > 0) {
		...
		prizeNames.forEach((el, i) => {
			if (names[i]) el.textContent = names[i]
		})
	}

	// Update crash screen
	const crashTitle = document.querySelector('#screen-crash .screen-title')
	if (crashTitle) crashTitle.textContent = t('crashTitle')

	const crashSubtitle = document.querySelector('#screen-crash .screen-subtitle')
	if (crashSubtitle) crashSubtitle.textContent = t('crashSubtitle')

	const betLabel = document.querySelector('#screen-crash label')
	if (betLabel) betLabel.textContent = t('bet')

	const playButton = document.getElementById('crash-play-btn')
	if (playButton) playButton.textContent = t('play')

	const cashoutButton = document.getElementById('crash-cashout-btn')
	if (cashoutButton) cashoutButton.textContent = t('cashOut')

	const yourBetLabel = document.querySelector(
		'#screen-crash .crash-info-item:nth-child(1) .crash-info-label',
	)
	if (yourBetLabel) yourBetLabel.textContent = t('yourBet')

	const potentialWinLabel = document.querySelector(
		'#screen-crash .crash-info-item:nth-child(2) .crash-info-label',
	)
	if (potentialWinLabel) potentialWinLabel.textContent = t('potentialWin')

	// Update bonuses screen
	const bonusesTitle = document.querySelector('#screen-bonus .screen-title')
	if (bonusesTitle) bonusesTitle.textContent = t('bonusesTitle')

	const bonusesSubtitle = document.querySelector(
		'#screen-bonus .screen-subtitle',
	)
	if (bonusesSubtitle) bonusesSubtitle.textContent = t('bonusesSubtitle')

	const promoInput = document.getElementById('promo-input')
	if (promoInput) promoInput.placeholder = t('enterPromo')

	const promoApply = document.getElementById('promo-apply')
	if (promoApply) promoApply.textContent = t('activate')

	const bonusTitle = document.querySelector('#screen-bonus .bonus-title')
	if (bonusTitle) bonusTitle.textContent = t('promoSection')

	const bonusText = document.querySelector('#screen-bonus .bonus-text')
	if (bonusText) bonusText.textContent = t('promoText')

	// Update profile screen
	const inventoryTitle = document.querySelector(
		'#screen-profile .inventory-title',
	)
	if (inventoryTitle) inventoryTitle.textContent = t('profileTitle')

	const inventoryEmpty = document.querySelector(
		'#screen-profile .inventory-empty',
	)
	if (inventoryEmpty) inventoryEmpty.textContent = t('profileEmpty')

	const referralTitle = document.querySelector('#screen-profile .ref-title')
	if (referralTitle) referralTitle.textContent = t('referralTitle')

	const referralText = document.querySelector('#screen-profile .ref-text')
	if (referralText) referralText.textContent = t('referralText')

	const createPromoBtn = document.querySelector(
		'#screen-profile .action-outline',
	)
	if (createPromoBtn) createPromoBtn.textContent = t('createPromo')

	// Update navigation
	const navButtons = document.querySelectorAll('.bottom-nav .nav-btn span')
	if (navButtons.length >= 4) {
		navButtons[0].textContent = t('wheelNav')
		navButtons[1].textContent = t('crashNav')
		navButtons[2].textContent = t('bonusesNav')
		navButtons[3].textContent = t('profileNav')
	}

	// Update modal
	const modalTitle = document.querySelector('#prize-modal .modal-title')
	if (modalTitle) modalTitle.textContent = t('congratulations')

	const priceLabel = document.querySelector('#prize-modal .modal-price')
	if (priceLabel)
		priceLabel.innerHTML = `${t('priceLabel')}: <span id="modal-prize-price">15</span> TON`

	const sellButton = document.getElementById('modal-sell')
	if (sellButton) sellButton.textContent = t('sell')

	const keepButton = document.getElementById('modal-keep')
	if (keepButton) keepButton.textContent = t('keep')
	...
	const closeButton = document.getElementById('close-settings-bottom')
